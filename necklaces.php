<?php require_once("inc/header.php") ?>
	
	<!-- Primary Page Layout
	================================================== -->
			<main class="cd-main-content" id="category-detail-page">
				<section class="cd-section home">
					<div class="cd-block home">
						<div class="portfolio-top parallax-window our-story" data-parallax="scroll" data-image-src="images/bg/our-collections.jpg" style="height:100% !important;">
								
							<div class="hero-top">	
								<div class="">	
									<div class="four columns">
										&nbsp;
									</div>
									<div class="seven columns services-list">
										<div class="hero-text">
											<div class="content">
												<h3 class="white">Necklaces</h3>
												<img src="images/divider.png">
												<div class="white hero-subtext-2">Exquisite & Certified Diamonds & Jewelries</div>
												<img src="images/divider-bottom.png" style="max-width:50%;width:90px;margin-top:10px;">
											</div>
										</div>	
									</div>	
									<div class="four columns">
										&nbsp;
									</div>	
								</div>		
							</div>
							
<!-- 							<a href="#scroll-link" class="scroll scroll-down-arrow"></a>				 -->
							
						</div>
					</div>
				</section>
				<section>
					<div class="cd-block">

					</div>
				</section>
				<div class="section-in padding-top-bottom">
					<!-- template -->
					<div class="container">
							<div class="one columns">
								&nbsp;
							</div>
							<div class="seven columns story-text-box" style="">
								<div>
									<div class="section-header-text">
										<h3>Trust Jangmi!</h3>	
										<div class="line-header"></div>
									</div>
									<div class="services-list">
										<p class="text-bottom-margin">Jangmi goes the extra mile for the customers to provide the best quality products.</p> 
									</div>	
									<div class="services-list text-center">
										<a href="our-story.php" class="button">LEARN MORE</a>
									</div>	
								</div>	
							</div>
							<div class="one columns">
								&nbsp;
							</div>
							<div class="six columns story-img-box">
								<img src="images/story1.jpg" class="img-responsive">
							</div>
							<div class="one columns">
								&nbsp;
							</div>
						</div>
					<div class="container" id="">
<!-- 						<div class="container ms-partialview-template" id="partial-view-1"> -->
					            <!-- masterslider -->
<!-- 						<div class="master-slider ms-skin-default" id="masterslider"> -->
											<div class="two columns">
												&nbsp;
											</div>
					            <div class="columns twelve " id=""><!-- no-margin-container -->
												<div class="row">
					                <div class="columns four cat-item 1">
<!-- 															<img src="../masterslider/style/blank.gif" data-src="categories/wedding-bands/images/1.jpg" alt="U Setting"/> -->
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n1.jpg" alt="Alba Pendant" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Alba Pendant</h3>
					                </div>
					                <div class="columns four cat-item 2">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n2.jpg" alt="BezelPendant 14k yellow gold" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
					                  <h3>BezelPendant 14k yellow gold</h3>  
					                </div>
					                <div class="columns four cat-item 3">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n3.jpg" alt="Bow Pendant" class="img-responsive"/>
																	</li>
																</ul>
															</div>
														</section>
														<h3>Bow Pendant</h3>
					                </div>
					                <div class="columns four cat-item 4">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n4.jpg" alt="Butterfly pendant 14k White Gold" class="img-responsive"/>
																	</li>
																</ul>
															</div>
														</section>
														<h3>Butterfly pendant 14k White Gold</h3>
					                </div>
					                 <div class="columns four cat-item 5">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n5.jpg" alt="Cluster Pendant 14K Rose Gold"  class="img-responsive"/>   
																	</li>
																</ul>
															</div>
														</section>
														<h3>Cluster Pendant 14K Rose Gold</h3>
					                </div>
					                <div class="columns four cat-item 6">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n6.jpg" alt="Droplet Pendant" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Droplet Pendant</h3>
					                </div>
					                <div class="columns four cat-item 7">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n7.jpg" alt="Fancy infinity diamond pendant with bar chain" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Fancy infinity diamond pendant with bar chain</h3>
					                </div>
					                <div class="columns four cat-item 8">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n8.jpg" alt="Fiore Pendant" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Fiore Pendant</h3>
					                </div>
					                <div class="columns four cat-item 9">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n9.jpg" alt="Five Diamond Bezel Pendant" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Five Diamond Bezel Pendant</h3>
					                </div>
					                <div class="columns four cat-item 10">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n10.jpg" alt="Hand Pave heart pendant" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Hand Pave heart pendant</h3>
					                </div>
					                <div class="columns four cat-item 11">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n11.jpg" alt="Pear shape pave" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Pear shape pave</h3>
					                </div>
					                <div class="columns four cat-item 12">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n12.jpg" alt="Round Brilliant Droplet Pendant" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Round Brilliant Droplet Pendant</h3>
					                </div>
					                <div class="columns four cat-item 13">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n13.jpg" alt="Solitare Pendant 14K White Gold" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Solitare Pendant 14K White Gold</h3>
					                </div>
					                <div class="columns four cat-item 14">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n14.jpg" alt="Three Heart Pendant" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Three Heart Pendant</h3>
					                </div>
					                <div class="columns four cat-item 15">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n15.jpg" alt="Fleur pendant" class="img-responsive"/> 
																	</li>
																	<li>
																		<img src="categories/necklaces/images/n15.2.jpg" alt="Fleur pendant" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Fleur pendant</h3>
					                </div>
					                <div class="columns four cat-item 16">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n16.jpg" alt="Forever Brillant Neckalce" class="img-responsive"/> 
																	</li>
																	<li>
																		<img src="categories/necklaces/images/n16.2.jpg" alt="Forever Brillant Neckalce" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Forever Brillant Neckalce</h3>
					                </div>
					                <div class="columns four cat-item 17">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n17.jpg" alt="Infini Brillant necklace" class="img-responsive"/> 
																	</li>
																	<li>
																		<img src="categories/necklaces/images/n17.2.jpg" alt="Infini Brillant necklace" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Infini Brillant necklace</h3>
					                </div>
					                <div class="columns four cat-item 18">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n18.jpg" alt="Reine Necklace" class="img-responsive"/> 
																	</li>
																	<li>
																		<img src="categories/necklaces/images/n18.2.jpg" alt="Reine Necklace" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Reine Necklace</h3>
					                </div>
					                <div class="columns four cat-item 19">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n19.jpg" alt="Tennis Necklace" class="img-responsive"/> 
																	</li>
																	<li>
																		<img src="categories/necklaces/images/n19.2.jpg" alt="Tennis Necklace" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Tennis Necklace</h3>
					                </div>
					                <div class="columns four cat-item 20">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n20.jpg" alt="Tennis Necklace II" class="img-responsive"/> 
																	</li>
																	<li>
																		<img src="categories/necklaces/images/n20.2.jpg" alt="Tennis Necklace II" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Tennis Necklace II</h3>
					                </div>
					                <div class="columns four cat-item 21">
														<section class="slider">
															<div class="flexslider">
																<ul class="slides">
																	<li>
																		<img src="categories/necklaces/images/n21.jpg" alt="Unknown" class="img-responsive"/> 
																	</li>
																	<li>
																		<img src="categories/necklaces/images/n21.2.jpg" alt="Unknown" class="img-responsive"/> 
																	</li>
																</ul>
															</div>
														</section>
														<h3>Unknown</h3>
					                </div>
												</div>
					            </div>
											<div class="two columns">
												&nbsp;
											</div>
					            <!-- end of masterslider -->
					</div>
					<!-- end of template -->    
				</div>
		
				<div class="main-wrapper">	
					
					


<?php require_once("inc/footer.php") ?>		